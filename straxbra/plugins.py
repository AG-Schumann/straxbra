...
class Records(strax.Plugin):
    ...
    def compute(self, raw_records):
        ...
        for xx in r:
            xx["time"] = xx["time"] - time_delay[ xx["channel"]]
...
@export
class NEvents(strax.Plugin):
    """
    just returns the number of Events
    """
    __version__ = "0.0.0.4"
    parallel = False
    depends_on = ('events',)
    ...
    def infer_dtype(self):
        _dtype = [
            (("Number of events in this run", "n"), np.int32)
        ]
...
@strax.takes_config(
    strax.Option('peak_area_min', default=20,
                 help='minimum area for a peak to potentially be a S1'),
    strax.Option('baseline_spacing', default=-1,
                 help='how far the inserted baseline peaks between peeks are spaced, set to negative value to turn off'),
    strax.Option('t_decay_default', default=0,
                 help='fallback value for decay time'),
...
@strax.takes_config(
    strax.Option('peak_area_min', default=20,
                 help='minimum area for a peak to potentially be a S1'),
    strax.Option('baseline_spacing', default=10,
                 help='how far the inserted baseline peaks between peeks are spaced, set to negative value to turn off'),
    # strax.Option('custom_settings', default={"p0_t_decay_default": 1500},
                 # help='custom parameters '),
                 
    strax.Option('t_decay_default', default=1500,
                 help='fallback value for decay time'),

)
@export
class EventFitsS1(strax.LoopPlugin):
    """
    stolen from SPKrypton
    """
    ...
    def compute_loop(self, event, peaks):
        ...
        try:
            ...
            r["fit_S1"] = fit_S1
            r["sfit_S1"] = sfit_S1
            r["decaytime_S1"] = rp["decaytime"]
            r["areas_S1"] = rp["areas"][:2]
            r["widths_S1"] = rp["widths"][:2]
            r["t_max_S1"] = rp["t_max"][:2]
            r["t_mid_S1"] = rp["t_mid"][:2]
...
@export
@strax.takes_config(
    strax.Option('min_areas', default=[20, 20, 25, 25],
...
class EventFitsSummary(strax.LoopPlugin):
    ...
    def compute_loop(self, event):
        ...
        widths[~np.isfinite(widths)] = -1

        
        # check if first S1 is an S1
        checks = (
              (areas >= self.config["min_areas"])
            & (areas <= self.config["max_areas"])
            & (widths >= self.config["min_widths"])
            & (widths <= self.config["max_widths"])
        )
        
        p0, p1 = self.config["S2_selection_parameters"]
        try:
            if areas[2] > 0:
        ...
        if (event["is_event"]==True) & (event["OK"]): #checking if there are two S1's and one or two S2's and if the final typical krypton fit went ok
            ...
            if event["s2_split"]==True:
                ...
                if event["time_signals"][2] == -1 or event["time_signals"][3] == -1:
                    ...
                    last_s2 = np.minimum( event["time_signals"][2], event["time_signals"][3])
...
class PeaksKryptonLabelled(strax.Plugin):
    ...
    dtype = [
        ...
        (("Time distance to the last S1 when the peak is associated to a krypton event", "time_distance1"), np.int64),
        (("Time distance to the last S2 when the peak is associated to a krypton event", "time_distance2"), np.int64),
    ...
    def compute(self, peaks, events):
        ...
        result = {
            ...
            'in_beforepeaks': 0*peaks['dt'],
            'in_afterpeaks': 0*peaks['dt'],
            'start_event': 0*peaks['time'],
            'end_event': 0*peaks['time'],
        ...
        for i,peak in enumerate(peaks):
            ...
            if i == 0:
                ...
                result["time_to_last"][i] = peak["time"] - result["endtime"][i-1]
            ...
            if len(pos_event) == 0:  #there is no event belonging to this peak
                result["is_krypton"][i] = False
                result["is_krypton_s1"][i] = False
                result["is_krypton_s2"][i] = False
                result["in_beforepeaks"][i] = False
                result["in_afterpeaks"][i] = False
                result['start_event'][i] = 0
                result['end_event'][i] = 0
                result['time_distance1'][i] = 0
                result['time_distance2'][i] = 0
                ...
                if pos_event["is_kryptonevent"] == True:
                    ...
                    if (( peak["time"] >= pos_event['end_s1']) & ( peak["time"] < pos_event['start_s2'])):
                        result["in_beforepeaks"][i] = True
                        result["time_distance1"][i] = peak["time"] - pos_event['end_s1']
                    else:
                        result["in_beforepeaks"][i] = False 
                        result["time_distance1"][i] = -1
                        
                    #checking for afterpeaks and calculating time_distance2
                    if (( peak["time"] >= pos_event['end_s2']) & ( peak["time"] < pos_event['endtime'])):
                        result["in_afterpeaks"][i] = True
                        result["time_distance2"][i] = peak["time"] - pos_event['end_s2']
                    else:
                        result["in_afterpeaks"][i] = False
                        result["time_distance2"][i] = -1
                
                else: # no krypton event
                    result["is_krypton"][i] = False
                    result["is_krypton_s1"][i] = False
                    result["is_krypton_s2"][i] = False
                    result["in_beforepeaks"][i] = False
